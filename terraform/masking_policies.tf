...
# this is role-based rather than a blanket restriction, and for why the
# policy is defined in GOVERNANCE.SECURITY rather than PROD_ANALYTICS.PUBLIC.
#
...
# any table/column here -- attaching requires the APPLY MASKING POLICY
# privilege on top of the USAGE grant in roles.tf, and that grant is
# deliberately deferred to Phase 6, when there is a real column to attach
# to (see docs/data_modeling_decisions.md ADR-001, "Note on attachment
# timing"). Once dim_customer_360 (or equivalent) exists with its
# contact_email/customer_email column (the CRM/ERP fields documented in
